...
Lets look at the variables that (age.first.hip, age.first.wrist, age.first.diab). I noticed that these variables have NAN when fracture in fact hasn't happened. This kind of missing mechanis is called missing by design, since the missingness relies on answer in another variable. However it is a bit tricky to see what value should we put for age when there has been no fracture.
...
Same can be done for the variables age.diab, age.liver, age.heart, however this means that we are introducing a lot of correlation in our variables and should be aware of it. 

We will put these variables in the dd.imputed data set. We will be suing these variables in model building but take note that these varaibles are not as reliable and will try to avoid including them in the model if the model performance is not improved drastically.
