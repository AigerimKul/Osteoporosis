...
We see that 168 people were classified as having a fracture when in fact they haven't. Lets take  alook ath the important variables.
...
system.time(model3<-randomForest(fracture ~ WTMEC2YR+age.first.spine+BMI+age,   data = dd.train, importance = TRUE, ntree =600, mtry = 3))
model3
```
...
test.pred<-predict(model3,dd.test)
train.pred<-predict(model3,dd.train)
...
model3$importance %>% round()

partialPlot(model2, dd.train, gender)
varImpPlot(model3)

r<-rfcv(select(dd.train, -c("fracture")),cv.fold= 3, dd.train$fracture, recursive = TRUE)
